/* js/hopper-flight.js
   Curierii Hopper care traverseaza o scena. Era legat de o singura sectiune
   (.story-split--hopper .story-split__copy); acum monteaza cate o instanta
   pe fiecare element cu [data-hopper-flight], fiecare cu starea lui, ca sa
   poata rula in acelasi timp pe story si pe sectiunea de produse din index.

   Logica de zbor e neschimbata: inaltimea scenei se imparte in cate un
   teritoriu pentru fiecare curier, ancorat la textul real, nu la padding.
   Ruleaza doar cat scena e vizibila si daca utilizatorul n-a cerut mai
   putina miscare. */
(() => {
  const stages = document.querySelectorAll('[data-hopper-flight]');
  if (!stages.length) return;

  stages.forEach(mount);

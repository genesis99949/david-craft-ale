(()=>{
  ...
  <div class="site-menu" id="shared-menu" role="dialog" aria-modal="true" aria-label="Menu" aria-hidden="true">
    ...
    <div class="menu-foot"><img src="assets/hopper-hero.png" alt="Hopper, the David Craft Ale hop mascot"><span class="logo-star">${star}</span><a href="terms.html" data-shared-menu-link>Terms and conditions</a><a href="privacy.html" data-shared-menu-link>Cookies and Privacy Policy</a><span>Drink Responsibly</span></div>
  </div>
  <header class="site-header" id="top">
    ...
    <a href="index.html#top" class="logo-hopper" aria-label="David Craft Ale — home"><img src="assets/hopper-hero.png" alt=""></a>
  ...
  if(skip) skip.insertAdjacentHTML('afterend',chrome); else document.body.insertAdjacentHTML('afterbegin',chrome);
  const menu=document.querySelector('#shared-menu'),open=document.querySelector('#shared-menu-open'),close=document.querySelector('#shared-menu-close');
  let returnFocus=null;
  const syncLock=()=>{document.body.style.overflow=menu.classList.contains('is-open')?'hidden':''};
  const openMenu=()=>{returnFocus=document.activeElement;menu.classList.add('is-open');menu.setAttribute('aria-hidden','false');syncLock();close.focus()};
  const closeMenu=()=>{if(!menu.classList.contains('is-open'))return;menu.classList.remove('is-open');menu.setAttribute('aria-hidden','true');syncLock();returnFocus?.focus()};
  open.addEventListener('click',openMenu);close.addEventListener('click',closeMenu);menu.querySelectorAll('[data-shared-menu-link]').forEach(link=>link.addEventListener('click',()=>{menu.classList.remove('is-open');document.body.style.overflow=''}));
  document.addEventListener('keydown',event=>{if(event.key==='Escape')closeMenu();if(event.key==='Tab'&&menu.classList.contains('is-open')){const focusable=[...menu.querySelectorAll('a,button')];const first=focusable[0],last=focusable.at(-1);if(event.shiftKey&&document.activeElement===first){event.preventDefault();last.focus()}else if(!event.shiftKey&&document.activeElement===last){event.preventDefault();first.focus()}}});
})();
